...
def panel_klavyesi():
    return ReplyKeyboardMarkup([
        ["🟢 TALEPLERİ AÇ", "🔴 TALEPLERİ KAPAT", "📊 TALEP DURUMU"],
        ["📣 GENEL DUYURU", "📋 TALEP VERMEYENLER", "🔔 EKSİKLERE HATIRLAT"],
        ["🚫 KULLANICI ENGELLE", "✅ ENGEL KALDIR", "❌ PANELİ KAPAT"]
...
async def panel_secim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if secim in {"📣 GENEL DUYURU", "📣 GENEL DUYURU GÖNDER"}:
        await update.message.reply_text(
...
def talep_vermeyen_kullanicilar():
    ...
    for row in users_sheet.get_all_values()[1:]:
        ...
        if engelli_mi(isim) or normalize_name(isim) in talep_verenler:
            continue
