...
def panel_klavyesi():
    return ReplyKeyboardMarkup([
        ["🟢 TALEPLERİ AÇ", "🔴 TALEPLERİ KAPAT"],
        ["📊 TALEP DURUMU", "📣 GENEL DUYURU GÖNDER"],
        ["🔔 TALEP VERMEYENLERE HATIRLAT"],
...
async def panel_secim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if secim == "🔔 TALEP VERMEYENLERE HATIRLAT":
        await update.message.reply_text(
            "🔔 HATIRLATMA\n\nAktif dönemde henüz mesai talebi oluşturmamış, kayıtlı ve engelli olmayan kullanıcılara hatırlatma gönderilecek.\n\nGönderilsin mi?",
...
async def toplu_mesaj_gonder(context, mesaj, sadece_talep_vermeyenler=False):
    ...
    if sadece_talep_vermeyenler:
        for row in aktif_talep_sheet().get_all_values()[1:]:
